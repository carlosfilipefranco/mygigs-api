...
async function getMultiple(page = 1, search = null) {
	...
	const rows = await db.query(`SELECT gig.id, gig.date, artist.name as artist, artist.image, venue.name as venue, city.name as city FROM gig INNER JOIN artist ON gig.artist_id = artist.id INNER JOIN venue ON gig.venue_id = venue.id INNER JOIN city ON gig.city_id = city.id ${searchQuery} ORDER by gig.date DESC LIMIT ${offset},${config.listPerPage}`);
...
async function create(gig) {
	...
	if (gig.date && gig.date != "*") {
		...
		insert = `INSERT INTO gig (artist_id, venue_id, city_id, date)  VALUES  ("${artist[0].id}", "${venue[0].id}", "${city[0].id}", "${date}")`;
	}
	const result = await db.query(insert);

	let message = "Error in creating gig";
